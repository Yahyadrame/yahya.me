...
const TerminalMode = ({ setTerminalMode, setUiType }: Props) => {
  ...
  const handleCommand = (cmd: string) => {
    ...
    pushLine({ text: `yhd@terminal:~$ ${trimmed}`, type: "success" });
    ...
    if (trimmed === "gui" || trimmed === "gui on" || trimmed === "exit") {
      pushLine({ text: "Passage au mode GUI...", type: "info" });
    ...
    if (isValidCommand(trimmed)) {
      ...
      pushLine({
        text: `'${trimmed}' n'est pas reconnu. Tapez 'help' pour voir les commandes.`,
        type: "error",
      });
    }
  };
    ...
    >
      {glow && (
        <div className="absolute inset-0 z-0 pointer-events-none">
          <div className="absolute -inset-20 bg-brand-caribbean opacity-10 blur-3xl rounded-full" />
        </div>
      )}
      ...
      >
        {history.map((line, i) => (
          <div
            ...
            className={clsx(
              ...
              line.type === "success" && "text-brand-caribbean",
              line.type === "info" && "text-brand-meadow"
            )}
          >
            {line.text}
            {line.component}
        ...
        >
          <span className="pr-2 text-brand-meadow">yhd@terminal:~$</span>
          <div className="relative flex-1 min-h-[1.5rem]">
            ...
            <div className="absolute inset-0 pointer-events-none flex items-center whitespace-pre">
              ...
              <span className="bg-brand-caribbean text-brand-deep animate-[pulse_1s_step-end_infinite] inline-block text-center shadow-sm" style={{ minWidth: "0.7em" }}>
